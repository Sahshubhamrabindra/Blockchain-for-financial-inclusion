  ...
  addBlock(transaction: Transaction, senderPublicKey: string, signature: Buffer) {
    ...
    if (isValid) {
      ...
      if (transaction.amount <= beforeBalance) {
        ...
        console.log(`After balance (after deduction): ${afterBalance}`);
        console.log(`Transaction successful. Deducted ${transaction.amount} from ${transaction.payee}'s account.`);
      } else {
...
app.post('/sendMoney', express.json(), (req, res) => {
  const { amount, payerPublicKey, payeePublicKey } = req.body;

  const senderWallet = new Wallet();
  const parsedAmount = parseFloat(amount);
  const transaction = new Transaction(parsedAmount, payerPublicKey, payeePublicKey);
  const sign = crypto.createSign('SHA256');
  sign.update(transaction.toString()).end();
